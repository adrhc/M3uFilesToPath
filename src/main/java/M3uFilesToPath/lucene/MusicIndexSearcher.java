...
public class MusicIndexSearcher {
	...
	public List<M3uPlaylistSong> searchSong(M3uPlaylistSong songToSearch) {
		...
		Float scoreForMatch = Float.valueOf((String) songMatch.getMisc("score"));
		Float scoreForFound, scoreDeviationPercentAgainstMatch;
		songsToRemove = new ArrayList<>();
		for (M3uPlaylistSong songFound1 : songs) {
			scoreForFound = Float.valueOf((String) songFound1.getMisc("score"));
	...
	public List<M3uPlaylistSong> searchSongInMemory(M3uPlaylistSong originalSong,
		...
		List<M3uPlaylistSong> songs = new ArrayList<>(2);
		songs.add(originalSong);
		songs.add(duplicateSong);
		Directory directory = MusicIndex.createInMemoryIndex(songs);
		List<Map<String, String>> songsFoundInMem = searchSongDocIndex(originalSong, directory, resultPerSong);
		List<M3uPlaylistSong> songsFound = new ArrayList<>();
					...
					if (topDocs.totalHits > 0) {
						...
						if (topDocs.totalHits > 0) {
							...
							if (topDocs.totalHits > 0) {
								...
								System.out.println("\n" + songToSearch);
								System.out.println(expression);
								System.out.println(topDocs.totalHits + " rezults (query): " + booleanQuery);
								return null;
							}
						}
					}
